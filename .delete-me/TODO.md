...
[x] 3. Add Perses backend as a new component 
        [x] 1. rhobs/konflux-coo
        [x] 2. konflux-release-data 
[ ] 4. Add Hermetic Build 
        1. all new uiplugins 0.4/6.1
          [x] Logging 6-1      
                [x] .tekton https://github.com/rhobs/konflux-coo/pull/726
                [x] ignore scripts + cherry pick: https://github.com/openshift/logging-view-plugin/pull/263
          [ ] Logging 6-0
                [x] .tekton https://github.com/rhobs/konflux-coo/pull/726
                [x] ignore scripts + cherry pick: https://github.com/openshift/logging-view-plugin/pull/263
          [ ] console-dashboard       https://github.com/openshift/console-dashboards-plugin/pull/60
                [x] .tekton
                [x] ignore scripts + cherry pick 
          [ ] troublehshooting       https://github.com/openshift/troubleshooting-panel-console-plugin/pull/86
                [x] .tekton
                [x] ignore scripts + cherry pick 
          [ ] tracing 0-3       https://github.com/openshift/distributed-tracing-console-plugin/pull/69
                [x] .tekton
                [x] ignore scripts + cherry pick 
         [ ] tracing  0-4     https://github.com/openshift/distributed-tracing-console-plugin/pull/69
                [x] .tekton
                [x] ignore scripts + cherry pick 
          [ ] perses-operator
                [x] .tekton
          ...
          [ ] perses
                [x] .tekton
                [] ignore scripts + cherry pick (PENDING REVIEW)
[ ] 5. Update Render_templates -- this allow the staging relases to build the /konflux-coo image (Gabriel to send me instructions)
        ...
        2. Add Perses_backend 
            ...
            Changes needed in Konflux coo CSV (Cluster service version) to complete the components on boarding: 
                ...
                (JZ) Reference from health analyzer : https://github.com/rhobs/konflux-coo/pull/719/files (Alberto)
                (JZ) Reference from all others: https://github.com/rhobs/konflux-coo/pull/656/files (Jan)
...
### Pending PR/MR
1. Fix Perses-operator autogen files 
[x] Add autogenerated files for perses-operator https://gitlab.cee.redhat.com/releng/konflux-release-data/-/merge_requests/4812

2. Fix EnterpriseContractPolicy 
[-] Update EnterpriseContractPolicy https://gitlab.cee.redhat.com/releng/konflux-release-data/-/merge_requests/4792

3. Add Perses (Backend) Component 
[x] Add perses in midstream /rhobs/konflux-coo https://github.com/rhobs/konflux-coo/pull/725
[x] Add perses in /konflux-release-data https://gitlab.cee.redhat.com/releng/konflux-release-data/-/merge_requests/4817

4. Hermetic Build - Logging 
[x] rhobs/konflux-coo .tekton configs https://github.com/rhobs/konflux-coo/pull/726
[X] logging-view-plugin - need to make changes against main and then cherry pick into release-6-1

5. Fix names of Perses and Perses-operator 
[x] /konflux-release-data https://gitlab.cee.redhat.com/releng/konflux-release-data/-/merge_requests/4833#9d8338353fa988ba95543f39ccd6c3590bd4c99a
...
docker run --name jz-node-perses -it registry.redhat.io/ubi9/nodejs-18:latest /bin/bash

docker run --name jz-perses --user root -it  brew.registry.redhat.io/rh-osbs/openshift-golang-builder:rhel_8_golang_1.23 /bin/bash


docker run -d --name jz-node-perses3 registry.redhat.io/ubi9/nodejs-18:latest
